...
const EmailVerificationSecondPage = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            axios.get(`https://8dd8-175-157-13-142.ngrok-free.app/check-verification-status?email=${email}`)
                .then(response => {
    ...
    const handleContinueSignup = async () => {
        try {
            await axios.delete(`https://8dd8-175-157-13-142.ngrok-free.app/delete-verification-info?email=${email}`);
    ...
    const handleResendLink = async () => {
        if (remainingChances > 0) {
            setIsLoading(true);
            try {
                await axios.post(`https://8dd8-175-157-13-142.ngrok-free.app/send-verification-email?email=${email}`);
