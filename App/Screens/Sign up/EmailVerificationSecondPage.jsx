...
const EmailVerificationSecondPage = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            axios.get(`http://192.168.248.230:8005/check-verification-status?email=${email}`)
                .then(response => {
    ...
    const handleContinueSignup = async () => {
        try {
            await axios.delete(`http://192.168.248.230:8005/delete-verification-info?email=${email}`);
    ...
    const handleResendLink = async () => {
        if (remainingChances > 0) {
            setIsLoading(true);
            try {
                await axios.post(`http://192.168.248.230:8005/send-verification-email?email=${email}`);
